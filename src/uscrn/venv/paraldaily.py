...
"""
This function is called by main elab. chain and it's used for atomic computing of the daily uncertainty.
The function itself call the uscrnDaily() and use the output parameters to fill the result fields in the DB.
...
def paraldaily(wbanno, site):
    ...
    try:
        (tconcat, dateconcat, tmaxconcat, tminconcat, tmeanconcat, terrpos, terrneg, tmaxerrpos, tmaxerrneg,
         tminerrpos, tminerrneg, tmeanerrpos, tmeanerrneg, srconcat, precconcat, surtconcat, thourarray, precerr,
         solraderr, snerr, resterr, snerrarray, tmaxsr, tmaxprec, tmaxrest, tminsr, tminprec, tminrest, prtacc, dlerr,
         leaderr, restol, prtcal) = \
            uscrnDaily(conn, wbanno)
        ...
        # +ve Random = sqrt(prtacc.^2+(precerr/sqrt(n)).^2)
        random_pos_operand = np.vstack([
            np.power(prtacc, 2),
            np.power((precerr / np.sqrt(1728)), 2),  # ATTENTION: n=1728 for average
        ])
        ...
        rows = zip_longest(datestr, tconcat, terrneg, terrpos,
                           tmaxerrneg.ravel().tolist(), tmaxerrpos.ravel().tolist(), tminerrneg.ravel().tolist(),
                           tminerrpos.ravel().tolist(), tmeanerrneg.ravel().tolist(), tmeanerrpos.ravel().tolist(),
                           random_pos, random_neg, sys_pos, sys_neg, quasi_sys_pos, quasi_sys_neg, fillvalue="0")
        ...
        cur.execute(sql.SQL("CREATE TEMP TABLE {} AS SELECT * FROM uscrn.unc_daily LIMIT 0").format(
        ...
        for row in rows:
            cur.execute(
                sql.SQL("INSERT INTO {} (wbanno, sitename, date, t, terr_p, terr_m, tmaxerr_p, tmaxerr_m, "
                        "tminerr_p, tminerr_m, tmeanerr_p, tmeanerr_m, random_p, random_m, sys_p, sys_m, quasisys_p, "
        ...
        cur.execute(
            sql.SQL("INSERT INTO uscrn.unc_daily (wbanno, sitename, date, t, terr_p, terr_m, tmaxerr_p, tmaxerr_m, "
                    "tminerr_p, tminerr_m, tmeanerr_p, tmeanerr_m, random_p, random_m, sys_p, sys_m, quasisys_p, "
                    "quasisys_m) "
                    "SELECT * FROM {}"
        ...
        print("Daily chain - processed: {}".format(site))
        logging.info("Daily chain - processed: {}".format(site))
